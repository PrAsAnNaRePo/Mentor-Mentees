...
- Some user'll answer vague to your questions, At that time you have to go upto 4 or 5 follow up questions. If you get that much detailes for the question, you just proceed with one follow-up question. So basically asking follow-up questions is based on the detailes in the answer that you got.
...
def get_verb_analysis(prompt):
    return json.loads(client.chat.completions.create(
        ...
        messages=[
            {
                'role': 'user',
                'content': f"Given a text, you have to find if there is any verbs present like {open('bow.txt', 'r').read()} etc,.\ncontent text:{prompt}." + """\nUse json format to respond like following:
\{
    "is_verb_present": <yes/no>,
    "verb": <The excat verb present in the sentence or none>
\}"""
            }
        ]
    ).choices[0].message.content)
...
if prompt := st.chat_input("What is up?"):
    ...
    verbs_analysis = get_verb_analysis(prompt=prompt)
    print(verbs_analysis)
    if verbs_analysis['is_verb_present'] == 'no':
        st.session_state.messages.append({"role": "user", "content": prompt})
    else:
        st.session_state.messages.append({"role": "user", "content": prompt + f"\n<verb>Found a verb {verbs_analysis['verb']}</verb>"})
        # prompt += f"\n`<verb>{verbs_analysis['verb']}</verb>`"
